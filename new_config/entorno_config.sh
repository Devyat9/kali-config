...
# Install alacritty, (dropdown bind ctrl+enter), tmux, bat, lsd 
sudo apt install alacritty tmux bat lsd golang neo4j pipx dhclient -y

# Install tdrop
git clone https://github.com/noctuid/tdrop && cd tdrop && sudo make install && cd ..

# sxhkd -> replaced with a shortcut
...
# Wallpaper
sudo cp desktop1.jpg /usr/share/backgrounds/kali-16x9/default
sudo cp desktop2.jpg /usr/share/backgrounds/kali/kali-ferrofluid-16x9.jpg
...
cp vmware-copy.desktop ~/.config/autostart/
...
# sudo mount -t virtiofs shareS /mnt/share
